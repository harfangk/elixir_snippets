defmodule LLRBTree do
  ...
  """
  @typep node :: {integer, any, node, node, boolean} | {:error, String.t}

  @spec put(integer, any, node) :: node
  def put(k, v, node) 
  ...
  @spec get(node, integer) :: any
  def get(node, k) 

  def get(_, nil), do: {:error, "Key cannot be nil"}
  def get(nil, _), do: nil
  def get(_, k) when not is_integer(k), do: {:error, "Key is not of integer type"}
  def get({node_k, node_v, left_child, right_child, _}, k) when k == node_k, do: node_v
  def get({node_k, _, left_child, right_child, _}, k) when k < node_k, do: get(k, left_child)
  def get({node_k, _, left_child, right_child, _}, k) when k > node_k, do: get(k, right_child)
  
  @spec delete(node, integer) :: node
  def delete(node, k)
  ...
  def delete({node_k, node_v, node_lc, node_rc, node_is_red} = node, k) do
    cond do
  ...
  defp do_delete({k, v, lc, rc, is_red} = node) do
    case rc do
      nil -> nil
      _ -> {min_key, get(rc, min_key), lc, delete_min(rc), is_red}
      with min_key = min_key_of(rc)
    end
  end

  @spec delete_max(node) :: node
  def delete_max(nil), do: nil
  def delete_max({k, v, lc, rc, is_red} = node) do
    node = 
  ...
  @spec delete_min(node) :: node
  def delete_min(nil), do: nil
  def delete_min({k, v, lc, rc, is_red} = node) do
    case lc do
      nil -> nil
  ...
  defp min_key_of({node_k, node_v, left_child, _, _} = node) do
    case left_child do
      nil -> node_k
  ...
  defp rotate_left({parent_k, parent_v, parent_lc, parent_rc, parent_is_red} = node) do
  ...
  defp rotate_right({parent_k, parent_v, parent_lc, parent_rc, parent_is_red} = node) do
  ...
  defp flip_color({parent_k, parent_v, parent_lc, parent_rc, parent_is_red} = node) do
    {lc_k, lc_v, lc_left_child, lc_right_child, lc_is_red} = parent_lc
    {rc_k, rc_v, rc_left_child, rc_right_child, rc_is_red} = parent_rc
  ...
  defp balance_tree({k, v, lc, rc, is_red} = node) do
    node = if is_rc_red(new_node) do rotate_left(node) else node end
    node = if is_lc_and_lc_lc_red(new_node) do rotate_right(node) else node end
    node = if is_lc_and_rc_red(new_node) do flip_color(node) else node end
  end
